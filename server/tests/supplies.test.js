...
describe('Gestión de insumos', () => {
  ...
  it('permite eliminar un insumo', async () => {
    ...
    const row = await get('SELECT id FROM supply_items WHERE id = ?', [created.body.id]);
    expect(row).toBeNull();
  });
});
...
describe('Recetas: vínculo entre platillos e insumos', () => {
  ...
  it('actualiza la cantidad si ya existe el vínculo (no duplica)', async () => {
    ...
    const links = await query('SELECT * FROM menu_item_supplies WHERE menu_item_id = ? AND supply_item_id = ?', [menuItemId, supplyId]);
  ...
  it('elimina un vínculo receta', async () => {
    ...
    expect(del.status).toBe(200);
    const links = await query('SELECT * FROM menu_item_supplies WHERE id = ?', [created.body.id]);
...
describe('Descuento automático de insumos al crear pedidos', () => {
  ...
  it('descuenta los insumos vinculados según la cantidad ordenada', async () => {
    ...
    const tortilla = await get('SELECT * FROM supply_items WHERE id = ?', [tortillaId]);
    const carne = await get('SELECT * FROM supply_items WHERE id = ?', [carneId]);
  ...
  it('no descuenta insumos sin receta vinculada al producto', async () => {
    ...
    const refresco = await run('INSERT INTO menu_items (category_id, name, price) VALUES (?, ?, ?)', [cat.lastInsertRowid, 'Coca-Cola', 20]);
    await request(app).post('/api/orders').set('Authorization', `Bearer ${token}`).send({
...
describe('Recuento semanal de insumos', () => {
  ...
  it('archiva la semana anterior y reinicia comprado/consumido al llegar una nueva semana', async () => {
    ...
    await run('UPDATE supply_items SET consumed = 120 WHERE id = ?', [supplyId]);
    restoreMon();

    const restoreNextMon = mockNow('2026-08-10', 9, 0); // el siguiente lunes
    const didReset = await checkWeeklyReset();
    expect(didReset).toBe(true);

    const history = await query('SELECT * FROM supply_week_history WHERE supply_item_id = ?', [supplyId]);
    ...
    const supply = await get('SELECT * FROM supply_items WHERE id = ?', [supplyId]);
  ...
  it('no reinicia si ya corrió esta semana', async () => {
    ...
    const second = await checkWeeklyReset();
  ...
  it('permite consultar el historial semanal de un insumo', async () => {
    ...
    const created = await request(app).post('/api/supplies').set('Authorization', `Bearer ${monToken}`).send({ name: 'Tortillas', unit: 'pieza', purchased: 500 });
    supplyId = created.body.id;
    await run('UPDATE supply_items SET consumed = 200 WHERE id = ?', [supplyId]);
    restoreMon();
...
describe('decrementSuppliesForOrder (unidad)', () => {
  ...
  it('ignora items sin menu_item_id', async () => {
    await decrementSuppliesForOrder([{ menu_item_id: null, quantity: 5, name: 'Personalizado', price: 10 }]);
    const tortilla = await get('SELECT consumed FROM supply_items WHERE id = ?', [tortillaId]);
    expect(tortilla.consumed).toBe(0);
  });

  it('descuenta 1 por unidad cuando la receta usa cantidad_por_unidad 1', async () => {
    await decrementSuppliesForOrder([{ menu_item_id: tacoId, quantity: 7 }]);
    const tortilla = await get('SELECT consumed FROM supply_items WHERE id = ?', [tortillaId]);
