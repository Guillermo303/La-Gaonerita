        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {categories.map(cat => (
            <button
              ...
              className="card-glow bg-white rounded-xl shadow-sm border border-ink-900/5 px-3 py-4 text-center hover:border-brand-400 hover:shadow-lg transition group"
            >
              <h2 className="font-display text-sm md:text-base font-extrabold text-ink-900 group-hover:text-brand-600 transition leading-tight">{cat.name}</h2>
              <p className="text-brand-500 text-[10px] font-bold uppercase tracking-wider mt-1">{cat.items.length} platillos</p>
            </button>
          ))}
        </div>
