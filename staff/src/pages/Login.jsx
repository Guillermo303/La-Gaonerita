...
const destinoPorRol = { admin: '/kitchen', cocina: '/kitchen', mesero: '/waiter' };
...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const destino = destinoPorRol[user.role];
      if (!destino) {
        logout();
        setError('Esta cuenta no tiene acceso al panel de personal.');
        return;
      }
      navigate(destino);
    } catch (err) {
    ...
    <div className="min-h-screen bg-cream-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-ink-400 mt-1">Panel de personal</p>
        </div>
