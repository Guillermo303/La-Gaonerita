...
router.get('/', authenticate, authorize('admin'), (req, res) => {
  const rows = query(`${SELECT_EMPLOYEES} ORDER BY u.active DESC, u.name ASC`);
...
router.post('/', authenticate, authorize('admin'), (req, res) => {
  ...
  run(`INSERT INTO employee_details (user_id, puesto, salario, periodo_pago, prestaciones, dias_laborales, fecha_contratacion)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
    [userId, puesto || '', parseFloat(salario) || 0, periodo_pago || 'semanal', JSON.stringify(prestaciones || []), JSON.stringify(dias_laborales || []), today()]);

  const row = get(`${SELECT_EMPLOYEES} AND u.id = ?`, [userId]);
...
router.put('/:id', authenticate, authorize('admin'), (req, res) => {
  ...
  const target = get('SELECT id FROM users WHERE id = ? AND role != \'cliente\'', [id]);
  ...
  run('UPDATE users SET name = COALESCE(?, name), phone = COALESCE(?, phone), role = COALESCE(?, role) WHERE id = ?', [name, phone, allowedRole, id]);

  const existingDetails = get('SELECT user_id FROM employee_details WHERE user_id = ?', [id]);
  if (existingDetails) {
    run(`UPDATE employee_details SET
  ...
  const row = get(`${SELECT_EMPLOYEES} AND u.id = ?`, [id]);
...
router.put('/:id/status', authenticate, authorize('admin'), (req, res) => {
  ...
  const existingDetails = get('SELECT user_id FROM employee_details WHERE user_id = ?', [id]);
  if (!existingDetails) {
    run(`INSERT INTO employee_details (user_id, fecha_contratacion) VALUES (?, ?)`, [id, today()]);
  }

  if (active) {
    run('UPDATE users SET active = 1 WHERE id = ?', [id]);
    run('UPDATE employee_details SET fecha_baja = NULL WHERE user_id = ?', [id]);
  } else {
    ...
    run('UPDATE employee_details SET fecha_baja = ? WHERE user_id = ?', [today(), id]);
  }
