  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-14 pb-32">
      <div className="text-center mb-14">
        ...
        <a href={DELIVERY_URL} target="_blank" rel="noopener noreferrer" className="btn-grow inline-block mt-5 bg-brand-500 text-white px-8 py-3 rounded font-bold uppercase tracking-widest text-sm hover:bg-brand-600">Ordenar a Domicilio 🛵</a>
      </div>
      {menuData.filter(cat => cat.items.length > 0).map(cat => (
        <Reveal key={cat.id} className="mb-14">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {cat.items.map(item => (
              <div key={item.id} className="card-glow group bg-white rounded-xl shadow-sm border border-ink-900/5 p-5">
                {item.image && (
